...
public sealed class SaDisplacedgeEffect : VideoEffectBase
{
    ...
    [AnimationSlider("F1", "px", 0.5, 4)]
    public Animation DetectionScale { get; } = new(1.5, 0.5, 4);
    ...
    public Animation Radius { get; } = new(48, 0, 256);

    [Display(Name = "変位強度", Description = "液状化で画素をずらす最大距離", Order = 4)]
    [AnimationSlider("F1", "px", 0, 200)]
    public Animation Strength { get; } = new(46, 0, 200);
    ...
    [AnimationSlider("F0", "", 1, 6)]
    public Animation TurbulenceDetail { get; } = new(4, 1, 6);

    [Display(Name = "渦の大きさ", Description = "小さいほど細かい渦、大きいほど大きくうねる流れになります", Order = 7)]
    [AnimationSlider("F0", "px", 20, 800)]
    public Animation SwirlSize { get; } = new(220, 20, 800);

    [Display(Name = "流速", Description = "渦アニメーションの速さ。0%で静止", Order = 8)]
    [AnimationSlider("F1", "%", 0, 400)]
    public Animation FlowSpeed { get; } = new(100, 0, 400);
